use crate::{model::
    {AnnotatedComponent, AnonymousIndividual, Build, ComponentKind, ForIRI, Kinded},
...
pub fn normalize_and_compare<A: ForIRI>(o1:Vec<AnnotatedComponent<A>>, o2: Vec<AnnotatedComponent<A>>) -> bool {
    normalize(o1).eq(&normalize(o2))
}

pub fn normalize_and_assert_eq<A: ForIRI>(o1:Vec<AnnotatedComponent<A>>, o2: Vec<AnnotatedComponent<A>>) {
        assert_eq!(normalize(o1), normalize(o2))
    }

pub fn simplify<A:ForIRI>(o:Vec<AnnotatedComponent<A>>) -> Vec<AnnotatedComponent<A>> {
    o.into_iter().filter(|ac| ac.kind() != ComponentKind::DocIRI).collect()
}


struct Reanonymize<A:ForIRI>{count:usize, b:Build<A>}
...
mod test {
    ...
    fn doc_iri() {
        ...
        let v = vec![
            DocIRI(b.iri("http://example.com")).into()
        ];
