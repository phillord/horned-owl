...
pub struct IRIMappedIndex {
    irindex: RefCell<BTreeMap<IRI, BTreeSet<RcT<AnnotatedAxiom>>>>,
}
...
impl IRIMappedIndex {
    ...
    fn axioms_as_ptr(
        &self,
        iri: IRI,
    ) -> *mut BTreeMap<IRI, BTreeSet<RcT<AnnotatedAxiom>>> {
        self.irindex
            .borrow_mut()
            .entry(iri)
    ...
    fn set_for_iri(&self, iri: IRI) -> Option<&BTreeSet<RcT<AnnotatedAxiom>>> {
        unsafe { (*self.irindex.as_ptr()).get(&iri) }
    }

    /// Fetch the axioms for given iri as a mutable ref.
    fn mut_set_for_iri(&mut self, iri: IRI) -> &mut BTreeSet<RcT<AnnotatedAxiom>> {
